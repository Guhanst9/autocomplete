...
def test_s4d_numerical_stability():
    ...
    print(f"  transition magnitude min: {transition_magnitude.min().item():.8f}")
    print(f"  transition magnitude max: {transition_magnitude.max().item():.8f}")
...
def test_causal_forward_pass():
    ...
    max_diff = (original_logits[:, :32] - changed_logits[:, :32]).abs().max().item()
    print(f"  prefix max abs diff: {max_diff:.8f}")
...
def test_checkpoint_reload():
    ...
    print(f"  reload max abs diff: {max_diff:.8f}")
